...
export default async function registerGameEventHandlers(
  roomId: string,
  gameManager: GMType,
  socketServer: Server
) {
  try {
    ...
    socketsInRoom.forEach((socket: any) => {
      socket.on(GAME.GAMESTATE_GET, () => {
        ...
        socketServer
          .to(roomId)
          .emit(GAME.GAMESTATE_CHANGED, gameManager.getGameState());
      });
      ...
      socket.on(GAME.RESOLUTION.QUEUE.CONFIRM_NO_TARGET, () => {
        console.log(
          "EVENT RECEIVED: ",
          GAME.RESOLUTION.QUEUE.CONFIRM_NO_TARGET
        );
      ...
      socket.on(GAME.RESOLUTION.QUEUE.CONFIRM_INTERRUPT, () => {
        console.log(
          "EVENT RECEIVED: ",
          GAME.RESOLUTION.QUEUE.CONFIRM_INTERRUPT
        );
      ...
      socket.on(
        GAME.RESOLUTION.QUEUE.CONFIRM_DISCARD,
        (discardIndex: number) => {
          console.log(
            "EVENT RECEIVED: ",
            GAME.RESOLUTION.QUEUE.CONFIRM_DISCARD
