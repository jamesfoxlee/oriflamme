...
export default function GameManager(): GameManager {
  ...
  const _checkForAdvanceToNextRound = (prevState: GameState) => {
    ...
    if (qri < queue.length) {
      ...
      const updatedTurnOrder = nextState.turnOrder
        .slice(1)
        .concat(nextState.turnOrder.slice(0, 1));
  ...
  const _requestTargets = (prevState: GameState) => {
    ...
    const { queue, queueResolutionIndex: qri } = nextState;
    const resolvingCard = _getTopCardInStack(queue, qri);
    const { targets, targetsNoneValid, targetsNothing, targetsSelf } =
    ...
    if (action.influenceChange) {
      switch (action.type) {
        ...
        case CARD_EFFECTS.ELIMINATE:
          nextState = _eliminate(
            targettedIndex,
            resolvingCard,
            action.influenceChange,
            prevState
          );
          break;
        case CARD_EFFECTS.GAIN_INFLUENCE:
          nextState = _gainInfluence(
            resolvingCard,
            action.influenceChange,
            prevState
          );
  ...
  const _checkForDiscardAfterResolution = (prevState: GameState) => {
    ...
    const toDiscard = cardHelper.getDiscardAfterResolution(
      resolvingCard,
      queue,
      qri
    );
  ...
  const _eliminate = (
    targettedIndex: number | null | undefined,
    resolvingCard: Card,
  ...
  const cardWasPlayed = (cardPlayed: Card, position: number) => {
    ...
    const updatedHand = player.hand.filter(
      (handCardId: string) => handCardId !== cardPlayed.id
    );
  ...
  const cardWasRevealed = (qri: number) => {
    ...
    const influenceChange = cardHelper.getInfluenceGainOnReveal(
      card,
      queue,
      qri
    );
  ...
  const targetWasConfirmed = (targettedIndex: number | null | undefined) => {
    console.log(
      "GameManager.targetWasConfirmed() at queue index: ",
      targettedIndex
    );
  ...
  const discardWasConfirmed = (discardIndex: number) => {
    console.log(
      "GameManager.discardWasConfirmed() at queue index: ",
      discardIndex
    );
